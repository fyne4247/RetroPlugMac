...
-- AUv2 (.component) for macOS hosts (Logic Pro, etc.)
function iplug2.project.auv2(fn, name)
	if _OPTIONS["emscripten"] ~= nil then return end

	local config = projectBase("auv2", name)
	kind "SharedLib"
	targetprefix "" -- avoid libRetroPlug_*.component
	...
	configuration { "macosx" }
		-- Bundle as Audio Unit component (not a flat dylib).
		targetextension ".component"
		xcodebuildsettings {
			["WRAPPER_EXTENSION"] = "component",
			["GENERATE_PKGINFO_FILE"] = "YES",
			-- mh_bundle uses -bundle; strip dylib-only version flags.
			["MACH_O_TYPE"] = "mh_bundle",
			["DYLIB_COMPATIBILITY_VERSION"] = "",
			["DYLIB_CURRENT_VERSION"] = "",
			["PRODUCT_BUNDLE_IDENTIFIER"] = "com." .. _config.plugin.author .. ".audiounit." .. _name,
			["INFOPLIST_FILE"] = "../../resources/" .. _name .. "-AU-Info.plist",
			["INSTALL_PATH"] = "$(HOME)/Library/Audio/Plug-Ins/Components",
			["SKIP_INSTALL"] = "NO",
			["COMBINE_HIDPI_IMAGES"] = "YES",
			["PRODUCT_NAME"] = _name,
			["EXECUTABLE_NAME"] = _name,
		}
		...
		-- Package flat mh_bundle into a real .component and install for Logic.
		postbuildcommands {
			"\"%{wks.location}/../../scripts/package_au.sh\" %{cfg.buildcfg}",
		}
...
function iplug2.project.auv3(fn, name)
	...
	-- Placeholder / incomplete; prefer project.auv2 for Logic Pro AUv2.
	local config = projectBase("auv3", name)
	kind "SharedLib"
